...
const JobDetail = ({ job, onClose, onApply }) => {
    ...
    return (
        <div className="fixed inset-0 z-[1000] bg-black bg-opacity-50 flex items-center justify-center px-4">
            <div className="w-full max-w-6xl max-h-[90vh] overflow-y-auto bg-white dark:bg-gray-800 text-gray-800 dark:text-white rounded-xl shadow-2xl">

                {/* Banner Section */}
                <div className="relative w-full bg-[#990e15] text-white text-center px-6 py-6 rounded-t-xl">
                    {/* Close Button */}
                    <button
                        onClick={onClose}
                        className="absolute top-2 right-2 md:top-4 md:right-6 text-2xl font-bold text-white hover:text-red-300 z-20"
                    >
                        ✕
                    </button>
                    ...
                    <div className="mt-3 space-y-1 text-sm">
                        <p>
                            Job Type: <span className="text-yellow-300">{job.job_type}</span>
                        </p>
                        <p>
                            Deadline:{" "}
                ...
                {/* Scrollable Content */}
                <div className="flex-1 overflow-y-auto p-6 grid grid-cols-1 md:grid-cols-3 gap-6 text-sm">
                    {/* Job Description */}
                    <div>
